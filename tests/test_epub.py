    ...
    def test_epub_has_opf_ncx_nav_and_chapters(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with zipfile.ZipFile(epub_path) as zf:
                ...
                css = next(n for n in names if n.endswith("eink.css"))
                css_text = zf.read(css).decode("utf-8")
                self.assertIn("text-align: justify", css_text)
                self.assertIn("text-indent:", css_text)
